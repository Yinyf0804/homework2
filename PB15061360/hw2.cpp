...
protected:
	...
	void Clear(MyExpression& expression);
	void Combine(MyExpression& expression, MyExpression expression1, MyExpression expression2);
	virtual bool HandleException(MyExpression& expression) = 0;
...
private:
	bool support_devision;
	bool support_exponentiation;
	bool isInRange(string a);
...
int QuestionSetGenerator::Setting(int quesnum, int opernum, string oper, int question_type, int lower_bound, int upper_bound) {
	...
	for (int i = 0; i < oper.size(); i++) {
		...
		else if (oper[i] == '^' && question_type == 0) {
			key[4] = 1;
		}
	}
	if (key[0] == 0 && key[1] == 0 && key[2] == 0 && key[3] == 0 && key[4] == 0) {
		flag = false;
		return -1;
	}
